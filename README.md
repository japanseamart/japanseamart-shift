...
- **シフト管理画面（Phase 1モバイル最適化完了）**
  - **3つの表示モード**
    - 📅 表ビュー: PC向けガントチャート（従来通り）
    - 👥 リストビュー: 従業員別カード表示（モバイル最適）
    - 📆 日別ビュー: 日付別カード表示（モバイル最適）
  - 自動休憩時間計算（6時間以上=60分）
  - 人件費リアルタイム計算
  - 予算管理・アラート機能
  - **月別予算参照機能** ←NEW
    - 月ごとに設定した予算を自動参照
    - 設定がない場合は店舗デフォルト予算を使用
    - 全店計モードでも月別予算を合計
  - シフト希望自動反映
  - 印刷機能（複数週選択可能）
  - **レスポンシブ対応**
    - デスクトップ: テーブル表示維持
    - モバイル: カード表示自動切替（md: 768px）
...
### レスポンシブ対応
- モバイル: 320px〜 (sm: 640px)
- タブレット: 768px〜 (md: 768px)
- デスクトップ: 1024px〜 (lg: 1024px)

### タッチターゲット設計
- 最小タッチターゲット: 44px (Apple HIG)
- 推奨タッチターゲット: 48px以上 (Material Design)
- 管理画面フォーム入力: 48px (h-12)
- モバイルボタン: 56px以上
